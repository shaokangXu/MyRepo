...
class Annotator:
    ...
    def poly_label(self, poly, label='', color=(128, 128, 128), txt_color=(0, 255, 0)):
        # if self.pil or not is_ascii(label):
        ...
        # else:
            ...
            # for draw center point, by zongkang 2023/7/17
            # x_center = int(np.mean(polygon_list[:, 0]))
            # y_center = int(np.mean(polygon_list[:, 1]))
            # center = (x_center, y_center)
            # cv2.circle(img=self.im_cv2, center=center, radius=5, color=color, thickness=-1)
            # label = ''
