...
def poly_filter(polys, h, w): 
    ...
    直接把超出大图范围的框给过滤掉，但有些框的坐标小于0, 没有被过滤掉
    其实，大于1280或者小于0的框都被截断了，这样会学习到不完整的目标
    Args:
        polys (array): (num, 8)
    ...
    # x_ctr, y_ctr = (x_max + x_min) / 2.0, (y_max + y_min) / 2.0 # (num)
    # keep_masks_ = (x_ctr > 0) & (x_ctr < w) & (y_ctr > 0) & (y_ctr < h) 

    keep_masks = (x_min > 0) & (x_max < w) & (y_min > 0) & (y_max < h)
    return keep_masks