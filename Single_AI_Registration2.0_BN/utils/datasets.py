...
ROOT = str(ROOT).split("/utils")[0]
print("ROOT = ", ROOT)
if str(ROOT) not in sys.path:
    sys.path.append(ROOT)  # add ROOT to PATH
# ROOT = Path(os.path.relpath(ROOT, Path.cwd()))  # relative

from utils.augmentations import Albumentations, augment_hsv, copy_paste, letterbox, mixup, random_perspective, CLA_hist_equalize
...
def create_dataloader(path, imgsz, batch_size, stride, names, single_cls=False, hyp=None, augment=False, affine_aug=False, cache=False, pad=0.0,
...
def img2label_paths(img_paths):
    ...
    # sa = "/images/", sb = "/labelTxt/"
    sa, sb = os.sep + 'images' + os.sep, os.sep + 'labelTxt' + os.sep  # /images/, /labels/ substrings
    # rsplit(str, 1)代表从右往左进行分割，1代表只分割一次，结果会舍弃str 
    # 返回label file path list
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self, path, cls_names, img_size=640, batch_size=16, augment=False, affine_augment=False, hyp=None, rect=False, image_weights=False,
        ...
        self.affine_augment = affine_augment
        self.hyp = hyp
        self.image_weights = image_weights
        self.rect = False if image_weights else rect
        # self.rect = False，才能使用mosaic augment
        ...
        # for single class val inference, by zongkang, 2023/7/19
        # if isinstance(cls_names, dict):
        #     self.cls_names = list(cls_names.values())
        self.cls_names = cls_names

        # get image file path list to self.img_files
        try:
            f = []  # image files
        ...
        # Check cache
        # get label file path list based self.img_files
        ...
        # labels(list[array]): n_imgs * array(num_gt_perimg, [cls_id, poly])
    ...
    def __getitem__(self, index):
        ...
        if self.augment:
            ...
            # HSV color-space for RGB image augment
            # augment_hsv(img, hgain=hyp['hsv_h'], sgain=hyp['hsv_s'], vgain=hyp['hsv_v'])

            # CLAHE for gray image
            img = CLA_hist_equalize(img) 

            # gamma变换
            if random.random() < hyp['gamma']:
...
def load_image_label(self, i):
    # loads 1 image from dataset index 'i', returns im, original hw, resized hw, resized label
    # self.imgs默认为None, 没有通过npy的方式load to disk
    im = self.imgs[i]
    # labels (array): (num_gt_perimg, [cls_id, poly]), shape = [n, 9], n代表每张图片的gt数量
    ...
    if im is None:  # not cached in ram
        ...
        if npy and npy.exists():  # load npy
            ...
            im = cv2.imread(path, 0)  # gray image
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        # place img in img4
        if i == 0:  # top left
            # 构造大图，大小为1280 * 1280, 是原图的4倍，初始全部填充为114，即灰色
            # img4 = np.full((s * 2, s * 2, img.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles

            # add by zongkang, 2022/8/25, for gray image 
            img4 = np.full((s * 2, s * 2), 114, dtype=np.uint8)  # base image with 4 tiles
            # 主要思想是固定右下角的坐标，改变左上角的坐标
            # 计算大图上的坐标，分别是左上角和右下角，左上角需要判断边界，若小图的w和h比xc和yc大，是从0开始；否则从xc-w和yc-h开始
            x1a, y1a, x2a, y2a = max(xc - w, 0), max(yc - h, 0), xc, yc  # xmin, ymin, xmax, ymax (large image)
            # 计算小图上的坐标，分别是左上角和右下角，判断 w 和 x2a - x1a的关系，但一定不可能超过w和h，可能全部取或者只取部分小图的区域
        ...
        # Labels
        # labels (array): (num_gt_perimg, [cls_id, poly])
        labels, segments = img_label.copy(), self.segments[index].copy() 
        if labels.size:
            # labels[:, 1:] = xywhn2xyxy(labels[:, 1:], w, h, padw, padh)  # normalized xywh to pixel xyxy format
            # 可能不会全取小图的区域，所以需要把label的坐标进行向右或者向下平移，因为在组图的时候是右下角的点是固定的
    ...
    # Concat/clip labels
    # 把4张图片的label concat到一起，变为[n, 9], n代表4张图片总共的gt数目
    ...
    h_filter = 2 * s
    w_filter = 2 * s
    # 过滤掉超出大图[1280, 1280]边界的部分，比如小于0或者大于1280的部分区域，超出边界的框直接删除，不进行clip
    labels_mask = poly_filter(polys=labels4[:, 1:].copy(), h=h_filter, w=w_filter)
    labels4 = labels4[labels_mask]
    # img4, labels4 = replicate(img4, labels4)  # replicate

    # Augment
    # 默认不会改变img4,labels4, segments4, segments4 is None
...
def autosplit(path='../datasets/coco128/images', weights=(0.8, 0.1, 0.1), annotated_only=False):
...
def verify_image_label(args):
    ...
    # get image's shape, label information, i.e., [class_id, x1, y1, x2, y2, x3, y3, x4, y4]
    # filter difficult = 2 label，避免学习到不完整的目标
...
def dataset_stats(path='coco128.yaml', autodownload=False, verbose=False, profile=False, hub=False):
    ...
    for split in 'train', 'val', 'test':
        ...
        'L1', 'L2', 'L3', 'L4', 'L5', 'L6'])  # load dataset
        for label in tqdm(dataset.labels, total=dataset.n, desc='Statistics'):
            # 统计class_id 0-16的出现的次数，其中label[:, 0]为每一幅图像的框的个数，代表每一个框的类别
            x.append(np.bincount(label[:, 0].astype(int), minlength=data['nc']))
        x = np.array(x)  # shape(6000x17)
        stats[split] = {'instance_stats': {'total': int(x.sum()), 'per_class': x.sum(0).tolist()},
                        'image_stats': {'total': dataset.n, 'unlabelled': int(np.all(x == 0, 1).sum()),
                                        'per_class': (x > 0).sum(0).tolist()},
                        'labels': [{str(Path(k).name): round_labels(v.tolist())} for k, v in
                                   zip(dataset.img_files, dataset.labels)]}
        print(stats[split]["labels"])
...
if __name__ == "__main__":
    hyp = check_yaml("/workspace/code/MaestroAlgoXrayImageDetection/data/hyps/obb/hyp.finetune_spine.yaml")
    ...
    train_loader, dataset = create_dataloader(
        "/workspace/data/obb_detection_data/c-arm_data/autosplit_val.txt", 1024, 1, 32, 
        # ['C1', 'C2', 'C3', 'C4', 'C5', 'C6', 'C7',
        # 'T1', 'T2', 'T3', 'T4', 'T5', 'T6',
        # 'T7', 'T8', 'T9', 'T10', 'T11', 'T12',
        # 'L1', 'L2', 'L3', 'L4', 'L5', 'L6'], single_cls=False,
        ['vertebrae', 'unkown'], single_cls=False,
        hyp=hyp, augment=True, affine_aug=True, cache=False, rect=True, rank=-1,
    ...
    # autosplit("/workspace/data/obb_detection_data/c-arm_data/images")

    # dataset_stats("/workspace/code/yolov5_obb/data/spine.yaml")
