...
def get_rotation_mat_single_axis( axis, angle ):
    ...
    if axis == 'y' : return np.array([[np.cos(angle), 0, np.sin(angle)],
                    [0, 1, 0],
                    [-np.sin(angle), 0, np.cos(angle)]])

    if axis == 'z' : return np.array([[np.cos(angle), -np.sin(angle), 0],
                    [np.sin(angle), np.cos(angle), 0],
                    [0, 0, 1]])



def get_rigid_motion_mat_from_euler( alpha, axis_1, beta, axis_2, gamma, axis_3, t_x, t_y, t_z ,rotation_center):
    ...
    rigid_motion_mat[:3, 3] = np.array([t_x, t_y, t_z])

    # Translate the rotation center
    translation_mat = np.eye(4)
    translation_mat[:3, 3] = -rotation_center

    # Translate back to the original position
    translation_back_mat = np.eye(4)
    translation_back_mat[:3, 3] = rotation_center

    rot_mat_with_translation = translation_back_mat @ rigid_motion_mat @ translation_mat


    return rot_mat_with_translation
