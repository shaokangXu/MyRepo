...
class SiddonGpu():
    ...
    def __init__(self, projector_info,
        ...
        if view=="AX":
            ...
            self.source[2] = self.movImageInfo['Volume_center'][2]- projector_info['focal_lenght'] *(1-CT2DRR_rate_AX)
            #DRRorigin
            self.DRRorigin[0] = self.movImageInfo['Volume_center'][0] - projector_info['DRR_pp'] - self.drrspacing*(self.drrsize - 1.) / 2.
            self.DRRorigin[1] = self.movImageInfo['Volume_center'][1] - projector_info['DRR_pp'] - self.drrspacing*(self.drrsize - 1.) / 2.
            self.DRRorigin[2] = self.movImageInfo['Volume_center'][2] + projector_info['focal_lenght'] *CT2DRR_rate_AX
            #plane Angle
            angle = [0, 0, 0]
        ...
        if view == "AP":
            # -------AP--------
            CT2DRR_rate_AP = round(random.uniform(1/4,3/8),2)
            #source
            self.source[0] = self.movImageInfo['Volume_center'][0]
            self.source[1] = self.movImageInfo['Volume_center'][1] - projector_info['focal_lenght']  *(1-CT2DRR_rate_AP)
            self.source[2] = self.movImageInfo['Volume_center'][2]
            #DRRorigin
            self.DRRorigin[0] = self.movImageInfo['Volume_center'][0] - projector_info['DRR_pp'] - self.drrspacing * (self.drrsize - 1.) / 2.
            self.DRRorigin[1] = self.movImageInfo['Volume_center'][1]  + projector_info['focal_lenght'] *CT2DRR_rate_AP
            self.DRRorigin[2] = self.movImageInfo['Volume_center'][2] - projector_info['DRR_pp'] - self.drrspacing * (self.drrsize- 1.) / 2.
            # plane Angle
            angle = [90, 0, 0]
        ...
        if view == "LAT":
            # -------LAT--------
            CT2DRR_rate_LAT = round(random.uniform(1/4,3/8),2)
            #source
            self.source[0] = self.movImageInfo['Volume_center'][0] - projector_info['focal_lenght'] *(1-CT2DRR_rate_LAT)
            self.source[1] = self.movImageInfo['Volume_center'][1]
            self.source[2] = self.movImageInfo['Volume_center'][2]
            #DRRorigin
            self.DRRorigin[0] = self.movImageInfo['Volume_center'][0] + projector_info['focal_lenght'] *CT2DRR_rate_LAT
            self.DRRorigin[1] = self.movImageInfo['Volume_center'][1] - projector_info['DRR_pp'] - self.drrspacing* (self.drrsize - 1.) / 2.
            self.DRRorigin[2] = self.movImageInfo['Volume_center'][2] - projector_info['DRR_pp'] - self.drrspacing * (self.drrsize- 1.) / 2.
            # plane Angle
            angle = [90, 90, 0]
    ...
    def compute(self, transform_parameters,threshold):
        ...
        Tr = rm.get_rigid_motion_mat_from_euler(rotz, 'z', rotx, 'x', roty, 'y', tx, ty, tz,rotation_center=self.movImageInfo['Volume_center'])
