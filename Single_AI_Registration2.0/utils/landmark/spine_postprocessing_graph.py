...
class SpinePostprocessingGraph(object):
            ...
            for next in nexts:
                ...
                for i in range(len(local_heatmap_maxima[curr])):
                    for j in range(len(local_heatmap_maxima[next])):
                        ...
                        """
                        self.unary_term = 0.2 * landmark.value + 2  --> landmark.value more large, result more large, less important
                        self.pairwise_term based on the relative landmark offset and distance 
                        """
                        # for c-arm x-ray ues original weight
                        weight = - (self.unary_term(curr_landmark) + self.pairwise_term(curr_landmark, next_landmark, curr, next))
                        # add by zongkang, 2023/7/5, fix the landmark.value not accurate
                        # weight = - self.pairwise_term(curr_landmark, next_landmark, curr, next)
    ...
    def path_to_landmarks(self, path, local_heatmap_maxima):
        ...
        """
        landmarks = [Landmark(coords=[np.nan] * 2, is_valid=False) for _ in range(self.num_landmarks)]
        for node in path:
            if node == 's' or node == 't':
                continue
    ...
    def solve_local_heatmap_maxima(self, local_heatmap_maxima):
        ...
        # for debug
        """
        minWPath4 = min([path for path in nx.all_simple_paths(G, 's', 't') if all(n in path for n in (list(G.nodes)[0]))], 
