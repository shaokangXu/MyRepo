...
class HeatmapTest(object):
    ...
    def get_landmarks(self, prediction_np, output_spacing=[0.418,0.418]):
        """
        ...
        np.rollaxis转换某个轴的顺序, 类似于np.transpose(array, (3, 0, 1, 2))
        也就是将预测的结果按照通道维度进行展开
        :param prediction_np: The np array to transform and shape is [128, 96, 129, 26]
        :param reference_sitk: The reference sitk image.
        :param output_spacing: The output spacing of the np array.
        :param transformation: The transformation. If transformation is None, the prediction np array will not be transformed.
        :return: A list of Landmark objects.
        """
        # image shape is [128, 96, 128] 按照每个通道进行计算landmark的坐标，总共有26个通道，即26个label

        return [self.get_landmark(image, output_spacing) for image in np.rollaxis(prediction_np, 0)]

    def get_multiple_maximum_coordinates(self, image):
        """
        Return local maxima of the image that are larger than self.min_max_value. 
        If self.smoothing_sigma > 0, perform Gaussian smoothing to reduce the number of local maxima.
    ...
    def get_multiple_maximum_coordinates_min_max_distance(self, image):
        ...
        If the local maximum value absolute_max_value > self.min_max_value, 
        also return other local maxima that are at least self.min_max_distance apart, 
        while having a value > absolute_max_value * self.multiple_min_max_value_factor.
        :param image: Heatmap image.
        :return: List of value, coord tuples of local maxima.
        """
        value_coord_pairs = []
    ...
    def get_landmark(self, image,output_spacing):
        """
