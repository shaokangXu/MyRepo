...
class LandmarkVisualizationBase(object):
    ...
    def __init__(self, dim=3,
        ...
        # TODO: bug fix when self.dim == 3, self.flip_y_axis is True, this generate a y axis invert error in saved image
    ...
    def prepare_image_canvas_list(self, image_sitk):
        ...
        if self.dim == 2:
            ...
            # 2024/1/17 by zongkang, fit to spine MR landmark localization
            return self.prepare_image_canvas([image_np[int(image_np.shape[0] / 2), :, :], image_np[:, int(image_np.shape[1] / 2), :], image_np[:, :, int(image_np.shape[2] / 2)]])
