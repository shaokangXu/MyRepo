...
class MainLoopBase(object):
    ...
    def save_model(self):
        ...
        """
        # 这样保存在onnx convert to trt时候会报错 saved_model convert to onnx只是有warnings
        # 可能的原因是 traing参数没有被设置 默认是False
        # 可尝试 tf.TensorSpec(tf.contant(False), tf.bool, name='training')
        new_save_path = os.path.join(self.output_folder_handler.path('weights'), str(self.current_iter + 1))
        if not os.path.exists(new_save_path):
            os.makedirs(new_save_path)
        tf.saved_model.save(self.model, new_save_path, signatures=self.model.call.get_concrete_function(
                            tf.TensorSpec([1, 1, None, None, None], tf.float16), tf.TensorSpec(None, tf.bool, name='training')))
        """
    ...
    def run(self):
        ...
        try:
            self.init_all()
            if self.load_model_filename is not None and self.load_model_filename != '':
                self.load_model()
