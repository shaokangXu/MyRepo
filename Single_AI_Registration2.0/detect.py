...
LastEditTime: 2024-10-18 11:41:38
...
#---------Detect--------------
weights='../Single_AI_Registration2.0/checkpoint_model/best.pt' # model.pt path(s)
...
def detect(imgs,view,SavePath,DrawImage=False):
    ...
    #如果需要保存画上检测框的图片，可以取消下方注释
    #循环为每个脊椎画上框
    if DrawImage:
        for i in range(pred_poly.shape[0]):

            if view=="AP":
                point1 = (int(pred_poly[i,0]), int(pred_poly[i,1]))
                point2 = (int(pred_poly[i,2]), int(pred_poly[i,3]))
                point3 = (int(pred_poly[i,4]), int(pred_poly[i,5]))
                point4 = (int(pred_poly[i,6]), int(pred_poly[i,7]))
            else:
                point1 = (int(pred_poly[i,0]), int(pred_poly[i,1]))
                point2 = (int(pred_poly[i,2]), int(pred_poly[i,3]))
                point3 = (int(pred_poly[i,4]), int(pred_poly[i,5]))
            ...
            # 在output_image上绘制边框
            cv2.polylines(image, [np.array(polygon)], isClosed=True, color=255, thickness=1)
        # 保存绘制了框的图像
        from PIL import Image
        image = Image.fromarray(image.astype(np.uint8))
        image.save("Output/"+SavePath) 
    return  pred_poly
